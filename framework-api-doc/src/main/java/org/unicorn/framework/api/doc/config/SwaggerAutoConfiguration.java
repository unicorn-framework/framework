...
public class SwaggerAutoConfiguration implements BeanFactoryAware {
    ...
    @Bean
    @ConditionalOnProperty(name = {"spring.swagger.enable", "spring.swagger.security.filter-plugin"}, havingValue = "true")
    ...
    public List<Docket> createRestApi(SwaggerProperties swaggerProperties) {
        ...
        if (swaggerProperties.getGroups().size() == 0) {
            //创建默认分组
            SwaggerProperties.GroupInfo groupInfo = BeanMapping.map(swaggerProperties, SwaggerProperties.GroupInfo.class);
            swaggerProperties.getGroups().put(DEFAULT_GROUP_NAME, groupInfo);
        }
        // 分组创建Docket
        for (String groupName : swaggerProperties.getGroups().keySet()) {
            ...
            ApiInfo apiInfo = genApiInfo(swaggerProperties, groupInfo, contact);
            // base-path处理
            // 当没有配置任何path的时候，解析/**
            ...
            Docket docket = genDocket(swaggerProperties, groupName, groupInfo, apiInfo, basePath, excludePath);
     ...
     * @return
     */
    private Docket genDocket(SwaggerProperties swaggerProperties, String groupName, SwaggerProperties.GroupInfo groupInfo, ApiInfo apiInfo, List<Predicate<String>> basePath, List<Predicate<String>> excludePath) {
     ...
     * @return
     */
    private ApiInfo genApiInfo(SwaggerProperties swaggerProperties, SwaggerProperties.GroupInfo groupInfo, SwaggerProperties.Contact contact) {
